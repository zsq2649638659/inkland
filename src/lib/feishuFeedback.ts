...
export function isFeishuFeedbackConfigured(): boolean {
...
async function fetchJson(url: string, init: RequestInit): Promise<FeishuResponse> {
  ...
  try {
    ...
    if (!response.ok) {
      throw new FeishuApiError(payload?.msg || `飞书接口请求失败（HTTP ${response.status}）`, response.status);
    }
    if (payload?.code !== 0) {
      throw new FeishuApiError(payload?.msg || "飞书接口返回失败", response.status);
    }
    return payload;
  } catch (error) {
...
async function getTenantAccessToken(config: FeishuConfig): Promise<string> {
  ...
  if (!token) throw new FeishuApiError("飞书没有返回访问令牌", 502);
  tokenCache = { token, expiresAt: Date.now() + Math.max(60, (payload.expire || 7_200) - 60) * 1_000 };
  return token;
}
...
async function createRecord(config: FeishuConfig, fields: Record<string, string>): Promise<FeishuRecord> {
  const payload = await bitableRequest(config, `/bitable/v1/apps/${encodeURIComponent(config.appToken)}/tables/${encodeURIComponent(config.tableId)}/records`, {
    method: "POST",
...
async function updateRecord(config: FeishuConfig, recordId: string, fields: Record<string, string>): Promise<FeishuRecord> {
  const payload = await bitableRequest(config, `/bitable/v1/apps/${encodeURIComponent(config.appToken)}/tables/${encodeURIComponent(config.tableId)}/records/${encodeURIComponent(recordId)}`, {
    method: "PUT",
    body: JSON.stringify({ fields }),
  });
  const record = payload.data?.record;
...
async function writeRecord(config: FeishuConfig, feedback: FeedbackRecord, profile: ProfileRecord | null): Promise<string> {
  ...
  if (existing?.record_id) {
    const record = await updateRecord(config, existing.record_id, fields);
    return record.record_id;
  }
  const record = await createRecord(config, buildFields(feedback, profile));
  return record.record_id;
}

async function markSyncFailure(database: FeedbackDatabase, feedback: FeedbackRecord, error: unknown): Promise<void> {
  const attempts = Number(feedback.feishu_sync_attempts || 0) + 1;
  const { error: updateError } = await database.from("feedbacks").update({
    feishu_sync_status: "failed",
    feishu_sync_attempts: attempts,
...
async function syncOne(database: FeedbackDatabase, config: FeishuConfig, feedbackId: string): Promise<SyncResult> {
...
export async function syncFeedbackToFeishu(feedbackId: string, database?: FeedbackDatabase): Promise<SyncResult> {
  const config = getConfig();
  if (!config) return { status: "skipped", error: "飞书反馈同步尚未配置" };
  let db = database;
  try {
    db ||= createAdminClient();
    return await syncOne(db, config, feedbackId);
  } catch (error) {
    console.error("启动反馈飞书同步失败:", { feedbackId, error: safeErrorMessage(error) });
    return { status: "failed", error: safeErrorMessage(error) };
  }
}

export async function syncPendingFeedbacksToFeishu(): Promise<BatchSyncResult> {
  const config = getConfig();
  if (!config) return { status: "skipped", total: 0, synced: 0, failed: 0, skipped: true };

  const database = createAdminClient();
  const { data: pending, error } = await database
    .from("feedbacks")
    .select("id")
    .or("feishu_record_id.is.null,feishu_sync_status.in.(pending,failed)")
    .order("created_at", { ascending: true })
    .limit(50);
  if (error) throw new Error(`读取待同步反馈失败：${error.message}`);

  let synced = 0;
  let failed = 0;
  for (const item of pending || []) {
    const result = await syncOne(database, config, item.id);
    if (result.status === "synced") synced += 1;
    if (result.status === "failed") failed += 1;
  }
  return { status: "completed", total: pending?.length || 0, synced, failed };
}
