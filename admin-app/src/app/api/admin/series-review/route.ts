...
export async function POST(request: Request) {
  ...
  if (body.decision === "rejected" && !body.reason?.trim()) return NextResponse.json({ error: "打回连载时必须选择问题类型" }, { status: 400 });
  ...
  const { error } = await db.from("series").update({ review_status: approved ? "approved" : "rejected", review_reason: approved ? null : body.reason, reviewed_at: new Date().toISOString() }).eq("id", body.seriesId);
  ...
  if (!approved) {
    await db.from("notifications").insert({
      user_id: series.user_id, type: "system", actor_id: null, content: `你的连载《${series.name}》未通过本次审核。问题类型：${body.reason}。请修改连载名称或简介后重新提交。`, read: false,
      template_key: "series_review_rejected", related_entity_type: "series", related_entity_id: series.id,
      metadata: { action_url: `/create?series=${series.id}`, action_label: "查看问题并修改", issue_type: body.reason }, delivery_status: "sent", sent_at: new Date().toISOString(),
    });
  }
