...
export async function PATCH(request: Request) {
  ...
  const body = await request.json().catch(() => null) as { reportId?: string; status?: string; source?: string } | null;
  ...
  const update = body.source === "comment" ? { status: body.status } : { status: body.status, resolved_at: new Date().toISOString(), resolved_by: user.id };
