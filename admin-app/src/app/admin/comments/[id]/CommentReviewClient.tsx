...
type HistoryItem = { id: string; status: string; priority: string; route_reason: string | null; screening_sources: string[] | null; submission_number: number | null; decision_reason: string | null; decided_by: string | null; decided_at: string | null; created_at: string; comment_snapshot: Record<string, unknown> | null };
...
export default function CommentReviewClient({ adminName, adminEmail, pendingCount, pendingCommentCount, queuePosition, queueTotal, previousCaseId, nextCaseId, reviewCase, comment, author, post, parentComment, findings, history }: {
  ...
  comment: { id: string; post_id: string; user_id: string; parent_id: string | null; paragraph_index: number | null; content: string; created_at: string } | null;
  author: { id: string; nickname: string | null } | null; post: { id: string; title: string | null; user_id: string } | null;
  parentComment: { id: string; user_id: string; content: string } | null; findings: Finding[]; history: HistoryItem[];
}) {
  ...
  const commentId = comment?.id || reviewCase.comment_id || String(snapshot.id || "—");
  const postId = comment?.post_id || reviewCase.post_id || String(snapshot.post_id || "—");
    ...
    <main className="admin-main"><header className="admin-topbar"><div className="admin-breadcrumb"><strong>管理后台 / 评论审核 / 详情</strong></div><div className="admin-top-actions"><button className="admin-btn admin-btn-light" type="button">全局搜索 <span className="admin-shortcut">⌘ K</span></button><button className="admin-btn admin-btn-light" type="button" onClick={() => window.location.reload()}>刷新</button><span className="admin-last-updated">上次更新 10:24</span><button className="admin-account-trigger" type="button">{adminName.slice(0, 8)} <span aria-hidden="true">⌄</span></button></div></header>
      <div className="admin-review-detail-page admin-comment-detail-page admin-detail-shell">
        ...
        <div className="admin-review-layout"><div className="admin-review-doc">
          ...
          <section className="admin-detail-panel admin-snapshot"><div className="admin-panel-title-row"><h2>评论快照（冻结）与上下文</h2></div><section className="admin-evidence-document"><div className="admin-document-label">【评论文本 · 冻结】</div><blockquote className="admin-comment-quote">{content}</blockquote><p className="admin-comment-context">所属位置：{post?.title || "未知作品"} · {parentId ? "回复评论" : paragraphIndex !== null ? `第 ${paragraphIndex} 段` : "作品评论区"}</p><p className="admin-comment-visibility">处理期间该评论对读者不可见；放行后立即恢复展示，删除后不可恢复。</p>{parentComment ? <div className="admin-comment-parent"><span>回复对象的原评论</span><b>{parentComment.content}</b></div> : null}</section></section>
