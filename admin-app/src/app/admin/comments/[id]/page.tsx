...
export default async function CommentReviewPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: reviewCase } = await db.from("comment_moderation_review_cases")
    .select("id, comment_id, post_id, author_id, parent_id, paragraph_index, status, priority, route_reason, screening_status, screening_sources, screening_result, rules_version, submission_number, comment_snapshot, decision_reason, decided_by, decided_at, created_at, updated_at")
  ...
  const [{ data: comment }, { data: author }, { data: post }, { data: parentComment }, { data: findings }, { data: history }] = await Promise.all([
    commentId ? db.from("comments").select("id, post_id, user_id, parent_id, paragraph_index, content, created_at").eq("id", commentId).maybeSingle() : Promise.resolve({ data: null }),
    authorId ? db.from("profiles").select("id, nickname").eq("id", authorId).maybeSingle() : Promise.resolve({ data: null }),
    postId ? db.from("posts").select("id, title, user_id").eq("id", postId).maybeSingle() : Promise.resolve({ data: null }),
    reviewCase.parent_id ? db.from("comments").select("id, user_id, content").eq("id", reviewCase.parent_id).maybeSingle() : Promise.resolve({ data: null }),
    ...
    db.from("comment_moderation_review_cases").select("id, status, priority, route_reason, screening_sources, submission_number, decision_reason, decided_by, decided_at, created_at, comment_snapshot").or(`comment_id.eq.${commentId || "00000000-0000-0000-0000-000000000000"},id.eq.${reviewCase.id}`).order("created_at", { ascending: true }),
  ]);
