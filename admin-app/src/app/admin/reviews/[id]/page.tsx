...
export default async function ReviewDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [{ data: version }, { data: post }, { data: findings }, { data: historyCases }, { data: historyVersions }] = await Promise.all([
    ...
    db.from("moderation_review_cases").select("id, post_version_id, status, priority, route_reason, submission_number, decided_by, decided_at, created_at").eq("post_id", reviewCase.post_id).order("created_at", { ascending: false }).limit(20),
    db.from("post_versions").select("id, post_id, version_number, submission_number, title, content, author_note, series_name, chapter_number, chapter_title, post_type, word_count, submitted_at, created_at").eq("post_id", reviewCase.post_id).order("version_number", { ascending: false }).limit(20),
  ]);
