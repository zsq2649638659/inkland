...
export default function AdminDashboard({ initialPosts, initialReports, initialFeedbacks, initialRules, rulesReady, loadErrors, initialView, adminName = "管理员", adminEmail }: {
  ...
  }, [initialPosts, initialReports, initialFeedbacks, initialRules]);
  ...
  const nav: Array<{ view: AdminView; label: string; icon: "file" | "flag" | "users" | "message" | "lock"; count?: number }> = [
    { view: "reviews", label: "作品审核", icon: "file", count: posts.length },
  ...
  const review = async (post: PostItem, decision: "approved" | "rejected") => {
    setBusy(post.id); setMessage("");
    const response = await fetch("/api/admin/review", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ postId: post.id, decision, reason: decision === "rejected" ? "内容需要调整后重新提交" : null }) });
    setBusy(null);
    if (!response.ok) { setMessage("审核操作失败，请稍后重试。"); return; }
    setPosts((items) => items.filter((item) => item.id !== post.id));
    setMessage(decision === "approved" ? "作品已通过审核。" : "作品已打回。模块3将补充问题位置标记。");
  };
  ...
  const postsView = <section className="admin-card admin-full-card"><div className="admin-card-heading"><div><div className="admin-heading-line"><span className="admin-section-dot dot-orange" /><h2>发布前人工审核</h2><span className="admin-count-pill">{filteredPosts.length} 条</span></div><p>打开详情页查看完整作品、原图和系统风险结果后，再决定放行或打回。</p></div></div><div className="admin-table">{filteredPosts.length === 0 ? <div className="admin-empty"><strong>没有符合条件的待审核作品</strong></div> : filteredPosts.map((post) => <div className="admin-table-row" key={post.id}><div className="admin-work-cell"><div className="admin-work-thumb">{post.title.slice(0, 1)}</div><div><strong>{post.title || "无标题"}</strong><span>{labels[post.post_type || ""] || "作品"}{post.review_reason ? ` · ${post.review_reason}` : ""}</span></div></div><span className="admin-author-cell">{post.author?.nickname || "未知作者"}</span><span className="admin-date-cell">{fmt(post.created_at)}</span><div className="admin-row-actions"><Link className="admin-btn admin-btn-primary" href={`/admin/reviews/${post.id}`}>查看审核</Link></div></div>)}</div></section>;
  ...
  const content = initialView === "reviews" ? postsView : initialView === "reports" ? reportsView : initialView === "feedbacks" ? feedbacksView : initialView === "rules" ? rulesView : <section className="admin-card admin-full-card"><div className="admin-coming-soon"><Icon name="users" /><strong>用户管理将在模块6接入</strong><span>当前不会使用假用户数据填充页面。</span></div></section>;
