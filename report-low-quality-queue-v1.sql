-- ============================================================
-- Inkland 飞书后续补全第 2 项：恶意举报第二级（低质量举报较低优先级队列）
-- 文件：report-low-quality-queue-v1.sql
-- 依赖：report-closure-v1.sql、report-notification-templates-v1.sql、
--       user-enforcement-module7-v1.sql、report-center-followup-v1.sql
-- 说明：幂等，可重复执行。在 Supabase SQL Editor 执行后：
--   1. 回填并持续维护 user_reporter_stats.reports_last_30d；
--   2. 自动评估长期低质量举报者（30 天举报量 + 成立率低/重复举报/集中举报，
--      低成立率单独不成立，需与量或重复信号组合）；
--   3. 低质量举报者新提交的举报自动进入 moderation_report_cases.low_quality_queue，
--      案件处理后自动重评并同步该举报者的其他待处理案件；
--   4. 后台举报中心新增“低质量队列”筛选，低质量队列案件排在普通案件之后。
-- ============================================================

BEGIN;

-- ------------------------------------------------------------
-- 1. 回填 30 天举报数（此前 submit_report_v1 未维护该列）
-- ------------------------------------------------------------

UPDATE public.user_reporter_stats urs
SET reports_last_30d = COALESCE(v.cnt, 0),
    updated_at = NOW()
FROM (
  SELECT u.user_id, COUNT(r.reporter_id) AS cnt
  FROM public.user_reporter_stats u
  LEFT JOIN (
    SELECT reporter_id, created_at FROM public.content_reports
    UNION ALL
    SELECT reporter_id, created_at FROM public.comment_reports
  ) r ON r.reporter_id = u.user_id AND r.created_at > NOW() - interval '30 days'
  GROUP BY u.user_id
) v
WHERE urs.user_id = v.user_id;

CREATE INDEX IF NOT EXISTS moderation_report_cases_low_quality_queue_idx
  ON public.moderation_report_cases (low_quality_queue, status, last_reported_at DESC)
  WHERE status IN ('pending', 'reviewing');

-- ------------------------------------------------------------
-- 2. 低质量举报者自动评估
--    判断条件（需同时满足量门槛与至少一个低质量信号，避免误伤）：
--      量门槛：30 天内举报 >= 3 次
--      信号 A：不成立举报 >= 3 且不成立率 >= 60%
--      信号 B：重复举报拦截次数 >= 3
--      信号 C：30 天内对同一目标用户举报 >= 3 次，且占其 30 天举报量 >= 50%
--    管理员通过后台手动标记/取消标记时，以管理员设置为准（low_quality_by 非 auto）。
-- ------------------------------------------------------------

CREATE OR REPLACE FUNCTION public.evaluate_reporter_low_quality(p_user_id UUID)
RETURNS BOOLEAN
LANGUAGE plpgsql
SECURITY DEFINER
...
DECLARE
  ...
  v_reports_30d INTEGER := 0;
  v_focused_count INTEGER := 0;
  v_invalid_rate NUMERIC := 0;
  v_manual_by TEXT;
  v_meta JSONB;
  v_low_quality BOOLEAN := FALSE;
  v_reason TEXT := '';
BEGIN
  SELECT * INTO v_stats FROM public.user_reporter_stats WHERE user_id = p_user_id;

  SELECT COUNT(*)::INTEGER INTO v_reports_30d
  FROM (
    SELECT 1 FROM public.content_reports cr
    WHERE cr.reporter_id = p_user_id AND cr.created_at > NOW() - interval '30 days'
    UNION ALL
    SELECT 1 FROM public.comment_reports cm
    WHERE cm.reporter_id = p_user_id AND cm.created_at > NOW() - interval '30 days'
  ) r;

  WITH report_targets AS (
    SELECT CASE
             WHEN cr.target_type = 'post' THEN (SELECT po.user_id FROM public.posts po WHERE po.id = cr.target_id)
    ...
    UNION ALL
    SELECT co.user_id, cm.created_at
    FROM public.comment_reports cm
    JOIN public.comments co ON co.id = cm.comment_id
    WHERE cm.reporter_id = p_user_id
  )
  SELECT COALESCE(MAX(cnt), 0) INTO v_focused_count
  FROM (
    SELECT COUNT(*) AS cnt
    FROM report_targets
  ...
  v_invalid_rate := COALESCE(v_stats.invalid_reports, 0)::numeric
    / GREATEST(COALESCE(v_stats.total_reports, 0), 1);

  IF v_reports_30d >= 3
     AND (
       (COALESCE(v_stats.invalid_reports, 0) >= 3 AND v_invalid_rate >= 0.6)
       OR COALESCE(v_stats.duplicate_attempts, 0) >= 3
       OR (
         v_focused_count >= 3
         AND v_focused_count::numeric / GREATEST(v_reports_30d, 1) >= 0.5
       )
     ) THEN
    v_low_quality := TRUE;
    v_reason := '30天内举报' || v_reports_30d || '次';
    IF COALESCE(v_stats.invalid_reports, 0) >= 3 AND v_invalid_rate >= 0.6 THEN
      v_reason := v_reason || '；成立率低';
    END IF;
    IF COALESCE(v_stats.duplicate_attempts, 0) >= 3 THEN
      v_reason := v_reason || '；重复举报较多';
    END IF;
    IF v_focused_count >= 3
       AND v_focused_count::numeric / GREATEST(v_reports_30d, 1) >= 0.5 THEN
      v_reason := v_reason || '；集中举报同一用户';
    END IF;
  END IF;

  v_manual_by := COALESCE(v_stats.metadata->>'low_quality_by', '');
  IF v_manual_by <> '' AND v_manual_by <> 'auto' THEN
    -- 管理员人工标记/取消标记优先，自动评估不覆盖
    v_low_quality := COALESCE((v_stats.metadata->>'low_quality')::BOOLEAN, FALSE);
    IF v_low_quality THEN
      v_reason := COALESCE(NULLIF(v_stats.metadata->>'low_quality_reason', ''), '管理员人工标记');
    END IF;
  END IF;

  v_meta := COALESCE(v_stats.metadata, '{}'::jsonb);
  IF v_manual_by = '' OR v_manual_by = 'auto' THEN
    IF COALESCE((v_meta->>'low_quality')::BOOLEAN, FALSE) <> v_low_quality
       OR COALESCE(v_meta->>'low_quality_reason', '') IS DISTINCT FROM v_reason THEN
      v_meta := v_meta || jsonb_build_object(
        'low_quality', v_low_quality,
  ...
  RETURN v_low_quality;
END;
$$;

REVOKE ALL ON FUNCTION public.evaluate_reporter_low_quality(UUID) FROM PUBLIC;

-- ------------------------------------------------------------
-- 3. 同步举报者的低质量标记到其全部待处理案件
-- ------------------------------------------------------------

CREATE OR REPLACE FUNCTION public.sync_reporter_low_quality(p_user_id UUID)
RETURNS INTEGER
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
AS $$
DECLARE
  v_low_quality BOOLEAN;
  v_updated INTEGER;
BEGIN
  v_low_quality := public.evaluate_reporter_low_quality(p_user_id);
  UPDATE public.moderation_report_cases c
  SET low_quality_queue = v_low_quality,
      updated_at = NOW()
  WHERE c.status IN ('pending', 'reviewing')
    AND EXISTS (
      SELECT 1 FROM public.content_reports cr WHERE cr.case_id = c.id AND cr.reporter_id = p_user_id
      UNION
      SELECT 1 FROM public.comment_reports cm WHERE cm.case_id = c.id AND cm.reporter_id = p_user_id
    );
...
REVOKE ALL ON FUNCTION public.sync_reporter_low_quality(UUID) FROM PUBLIC;

-- ------------------------------------------------------------
-- 4. submit_report_v1：维护 30 天举报数，并自动评估低质量队列
-- ------------------------------------------------------------
...
-- ------------------------------------------------------------
-- 5. admin_resolve_report_case：处理完成后重新评估举报者
-- ------------------------------------------------------------
...
BEGIN
  ...
  PERFORM public.sync_reporter_low_quality(p_user_id);
...
-- ------------------------------------------------------------
-- 8. 回填评估：对近 30 天有举报行为的用户重新计算低质量标记与案件队列
-- ------------------------------------------------------------
...
BEGIN
  FOR v_uid IN
    ...
    UNION
    SELECT reporter_id FROM public.comment_reports
    WHERE created_at > NOW() - interval '30 days'
  LOOP
    PERFORM public.sync_reporter_low_quality(v_uid);
  END LOOP;
END $$;

-- ============================================================
-- 校验查询（执行后可在 SQL Editor 直接运行确认）
-- SELECT to_regprocedure('public.evaluate_reporter_low_quality(uuid)');
-- SELECT to_regprocedure('public.sync_reporter_low_quality(uuid)');
-- SELECT to_regprocedure('public.submit_report_v1(text,uuid,text,text,jsonb)');
-- SELECT to_regprocedure('public.admin_resolve_report_case(uuid,text,text)');
-- SELECT to_regprocedure('public.admin_set_reporter_low_quality(uuid,boolean,text)');
-- SELECT to_regprocedure('public.admin_report_center_v1(text,text,text,text,boolean,boolean,boolean,boolean,text,integer)');
-- ============================================================
