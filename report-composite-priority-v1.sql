-- ============================================================
-- Inkland 飞书后续补全批次：举报综合优先级（3.19）+ 自动进入下一条（3.21）
-- 文件：report-composite-priority-v1.sql
-- 依赖：report-closure-v1.sql、report-center-followup-v1.sql、
--       report-low-quality-queue-v1.sql、report-high-risk-hide-v1.sql、
--       report-notification-improvements-v1.sql、user-report-actions-v1.sql
-- 说明：幂等，可重复执行。在 Supabase SQL Editor 执行后：
--   1. 重定义 admin_report_center_v1，举报案件综合计算有效优先级；
--   2. 综合分包含自动风险、风险分、服务异常、高危类别、不同举报人数、
--      近期确认违规、集中举报与等待时间；
--   3. 队列顺序：紧急 1 / 高 2 / 暂隐 3 / 普通 4 / 低质量 5，低质量不再排在普通之前；
--   4. 案件列表输出 composite_score 与 priority_factors，供后台与审计核验。
-- ============================================================
...
-- ------------------------------------------------------------
-- 7. admin_report_center_v1：综合优先级 + 暂时隐藏筛选 + 队列排序
-- ------------------------------------------------------------
...
BEGIN
  ...
  -- 内容举报案件：综合计算风险与举报者异常，按有效优先级排序
  WITH cd AS (
    SELECT
      c.*,
      s.object_snapshot,
      s.context_snapshot,
      COALESCE(NULLIF(c.auto_review_risk, ''), 'normal') AS v_auto_risk,
      COALESCE(c.risk_score, 0)::NUMERIC AS v_risk_score,
      COALESCE(c.report_count, 0) AS v_report_count,
      (COALESCE(c.service_error, FALSE) OR COALESCE(c.review_basis->>'service_error', 'false') = 'true') AS v_service_error,
      (
      ...
      ) AS v_recent_confirmed,
      (
        EXISTS (
          SELECT 1 FROM jsonb_array_elements_text(COALESCE(c.review_basis->'categories', '[]'::jsonb)) cat
          WHERE cat ~ '威胁|违法|诈骗|涉政|色情|赌博|毒品'
        )
        OR EXISTS (
          SELECT 1 FROM jsonb_array_elements(COALESCE(c.review_basis->'matched_keywords', '[]'::jsonb)) kw
          WHERE COALESCE(kw->>'category', '') ~ '威胁|违法|诈骗|涉政|色情|赌博|毒品'
        )
      ) AS v_high_risk_category,
      (EXTRACT(EPOCH FROM (COALESCE(c.last_reported_at, c.created_at) - COALESCE(c.first_reported_at, c.created_at))) / 86400) AS v_span_days,
      (EXTRACT(EPOCH FROM (NOW() - COALESCE(c.last_reported_at, c.created_at))) / 86400) AS v_wait_days
    ...
    SELECT
      ...
      ROUND((
        CASE v_auto_risk
          ...
          WHEN 'minor' THEN 45
          ELSE 20 END
        + LEAST(v_risk_score * 0.15, 15)
        + CASE WHEN v_service_error THEN 20 ELSE 0 END
        + CASE WHEN v_high_risk_category THEN 20 ELSE 0 END
        + CASE WHEN v_report_count >= 10 THEN 30
               WHEN v_report_count >= 5 THEN 20
               WHEN v_report_count >= 3 THEN 12 ELSE 0 END
        + CASE WHEN v_recent_confirmed >= 3 THEN 20
               WHEN v_recent_confirmed >= 1 THEN 10 ELSE 0 END
        + CASE WHEN v_report_count >= 3 AND v_span_days >= 0 AND v_span_days <= 1 THEN 12 ELSE 0 END
        + LEAST(GREATEST(v_wait_days, 0) * 2, 10)
      )::NUMERIC, 1) AS composite_score,
      jsonb_build_object(
        'auto_review_risk', v_auto_risk,
        'base_score', CASE v_auto_risk
          WHEN 'urgent' THEN 100
          WHEN 'high' THEN 75
          WHEN 'minor' THEN 45
          ELSE 20 END,
        'risk_score', v_risk_score,
        'risk_score_contribution', ROUND(LEAST(v_risk_score * 0.15, 15)::NUMERIC, 1),
        'service_error', v_service_error,
        'service_error_bonus', CASE WHEN v_service_error THEN 20 ELSE 0 END,
        'high_risk_category', v_high_risk_category,
        'high_risk_category_bonus', CASE WHEN v_high_risk_category THEN 20 ELSE 0 END,
        'report_count', v_report_count,
        'multi_reporter_bonus', CASE WHEN v_report_count >= 10 THEN 30
                                    WHEN v_report_count >= 5 THEN 20
                                    WHEN v_report_count >= 3 THEN 12 ELSE 0 END,
        'recent_confirmed_violations', v_recent_confirmed,
        'recent_violations_bonus', CASE WHEN v_recent_confirmed >= 3 THEN 20
                                       WHEN v_recent_confirmed >= 1 THEN 10 ELSE 0 END,
        'concentrated_reports', (v_report_count >= 3 AND v_span_days >= 0 AND v_span_days <= 1),
        'concentrated_bonus', CASE WHEN v_report_count >= 3 AND v_span_days >= 0 AND v_span_days <= 1 THEN 12 ELSE 0 END,
        'wait_days', ROUND(GREATEST(v_wait_days, 0)::NUMERIC, 1),
        'wait_time_bonus', ROUND(LEAST(GREATEST(v_wait_days, 0) * 2, 10)::NUMERIC, 1)
      ) AS priority_factors
    FROM cd
  )
  ...
  FROM (
    SELECT jsonb_build_object(
        ...
        'status', c.status,
        'priority', c.priority,
        'effective_priority', eff.effective_priority,
        ...
        'metadata', c.metadata,
        'target_title', CASE
          WHEN c.target_type = 'post' THEN COALESCE(c.object_snapshot->>'title', '')
          WHEN c.target_type = 'comment' THEN '评论于《' || COALESCE(c.context_snapshot->>'post_title', '未知作品') || '》'
          ELSE COALESCE(c.object_snapshot->>'nickname', '') END,
        'target_summary', left(
          btrim(regexp_replace(
            COALESCE(
              CASE WHEN c.target_type = 'post' THEN c.object_snapshot->>'content'
                   WHEN c.target_type = 'comment' THEN c.object_snapshot->>'content'
                   ELSE c.object_snapshot->>'bio' END,
              ''
            ),
            '!\[[^\]]*\]\([^)]*\)', '', 'g'
          )),
        ...
        'target_nickname', COALESCE(
          CASE WHEN c.target_type = 'post' THEN c.context_snapshot->>'author_nickname'
               WHEN c.target_type = 'comment' THEN c.context_snapshot->>'comment_author_nickname'
               ELSE c.object_snapshot->>'nickname' END,
          ''
        ),
    ...
    CASE
      WHEN eff.effective_priority = 'urgent' THEN 1
      WHEN eff.effective_priority = 'high' THEN 2
      WHEN COALESCE(c.hidden_for_review, FALSE) THEN 3
      WHEN COALESCE(c.low_quality_queue, FALSE) THEN 5
      ELSE 4 END AS queue_order,
    c.last_reported_at
    FROM scored c
    CROSS JOIN LATERAL (
      SELECT CASE
        WHEN c.v_auto_risk IN ('urgent', 'high') THEN c.v_auto_risk
        WHEN c.composite_score >= 80 THEN 'urgent'
        WHEN c.composite_score >= 55 THEN 'high'
        ELSE 'normal' END AS effective_priority
    ) eff
    WHERE (
        p_status = 'all'
      ...
      AND (p_priority = 'all' OR eff.effective_priority = p_priority)
      AND (p_target_type = 'all' OR c.target_type = p_target_type)
      AND (p_multi_report IS NULL OR (p_multi_report AND c.v_report_count >= 3))
      AND (
      ...
      AND (p_service_error IS NULL OR (p_service_error AND c.v_service_error))
      ...
      AND (
        ...
        OR COALESCE(c.object_snapshot->>'title', '') ILIKE '%' || v_query || '%'
        OR COALESCE(c.object_snapshot->>'nickname', '') ILIKE '%' || v_query || '%'
        OR COALESCE(c.context_snapshot->>'post_title', '') ILIKE '%' || v_query || '%'
